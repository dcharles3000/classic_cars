...
/// @param _tokenId, of the requested car
    function buyClassicCar(uint256 _tokenId) external payable {
        ...
        require(msg.sender != cars[_tokenId].owner, "Sorry, you can't buy your minted car");    // the buyer must not be the owner
...
/// @param _receiver, the address of the receiver of the car gift
    function giftClassicCar(uint256 _tokenId, address _receiver) external {
        require(_tokenId == cars[_tokenId].tokenId, "Car with the specific token doesn't exist");
        require(msg.sender == cars[_tokenId].owner, "Sorry, only car owner can gift car");
        require(msg.sender != _receiver, "Sorry, but you can't gift yourself your car");
...
/// @param _price, mew price of the car to sold
    function resellClassicCar(uint256 _tokenId, uint256 _price) external  {
        require(msg.sender == cars[_tokenId].owner, "Only owner can resell a cars");
        require(_tokenId == cars[_tokenId].tokenId, "cars not available");
